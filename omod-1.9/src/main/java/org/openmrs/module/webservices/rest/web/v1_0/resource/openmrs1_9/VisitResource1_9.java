...
import org.openmrs.module.webservices.rest.web.representation.FormRepresentation;
...
public class VisitResource1_9 extends DataDelegatingCrudResource<Visit> {
	...
	public static void setAttributes(Visit visit, Set<VisitAttribute> attributes) {
		for (VisitAttribute attribute : attributes) {
			...
			// We need to force the API to set valueReference on all new attributes before
			// any hibernate
			// auto flushes happen otherwise if they happen before we do so, the attributes
			// get rejected
	...
	private SimpleObject getVisits(RequestContext context, String patientParameter, String includeInactiveParameter,
		...
		Collection<VisitType> visitTypes = visitTypeParameter == null ? null
				: Arrays
						.asList(getVisitType(visitTypeParameter));
		boolean includeInactive = includeInactiveParameter == null ? true
				: Boolean.parseBoolean(includeInactiveParameter);
